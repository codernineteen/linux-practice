...
static void print_zone_dm(unsigned long index,
		const char *name,
		unsigned long long count,
		int percentage)
{
	printk("%s[%ld] called pxt4_file_write_iter()\
			%lld times (%d%%)\n",name, index, count, percentage);
}


DEFINE_DS_MONITORING(yechan_dm, get_thread_idx, get_thread_name, print_zone_dm);

//KTDEF(pxt4_file_write_iter);
unsigned long long file_write_iter_time, file_write_iter_count;
unsigned long long balanced_time, balanced_count;
...
static ssize_t pxt4_file_write_iter(struct kiocb *iocb, struct iov_iter *from) {
	...
	calclock(localclock, &file_write_iter_time, &file_write_iter_count);
	return ret;
}
