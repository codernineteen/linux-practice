...
DECLARE_DS_MONITORING(yechan_dm);
...
MODULE_ALIAS("ext3");
#define IS_PXT3_SB(sb) ((sb)->s_bdev->bd_holder == &ext3_fs_type)
...
#define MOPT_NO_PXT3	0x0200
#define MOPT_PXT4_ONLY	(MOPT_NO_PXT2 | MOPT_NO_PXT3)
			    ...
			    unsigned int *journal_ioprio, int is_remount)
{
	...
	if ((m->flags & MOPT_NO_PXT3) && IS_PXT3_SB(sb)) {
		pxt4_msg(sb, KERN_ERR,
...
static int pxt4_fill_super(struct super_block *sb, void *data, int silent)
{
	 ...
	 */
	if (!IS_PXT3_SB(sb) && !IS_PXT2_SB(sb) &&
...
extern unsigned long long balanced_time, balanced_count;
...
static void __exit pxt4_exit_fs(void)
{
	...
	printk("balance_dirty_pages_ratelimited is called %llu times and the time interval is %llu ns", balanced_count, balanced_time);
}
